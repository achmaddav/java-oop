...
public class Logic02Soal10Impl implements LogicInterface {
    ...
    public void isiArray(){
        ...
        int kolom = this.logic.n / 2;
        int baris = 0;

        for (int i = 0; i < this.logic.n; i++) {
            for (int j = 0; j < this.logic.n; j++) {
                if (i == 0 && j == 0) deret[i][j] = this.logic.n;
                else if (j == 0 && i <= this.logic.n / 2) deret[i][j] = deret[i - 1][j] - 2;
                else if (j == 0 && i > this.logic.n / 2) deret[i][j] = deret[i - 1][j] + 2;
            }
            ...
            if (i <= this.logic.n / 2) {
                for (int k = 1; k < kolom + 1; k++) {
                    ...
                    deret[i][this.logic.n - k] = deret[i][k - 1];
                }
                kolom--;
            } else {
            ...
            if (i > 0 && i <= this.logic.n / 2) {
                deret[i][this.logic.n / 2 - i] = 1;
                deret[i][this.logic.n / 2 + i] = 1;
                baris++;
            } else if (i > 0 && i > this.logic.n / 2) {
                baris--;
                deret[i][this.logic.n / 2 - baris] = 1;
                deret[i][this.logic.n / 2 + baris] = 1;
            }
        }
